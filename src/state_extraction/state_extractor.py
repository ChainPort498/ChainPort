            ...
            try:
                ...
                try:
                    ...
                    for curr_slot in range(0, math.ceil(string_length/64)):
                        if string_data_slot+curr_slot in storage.keys():
                            val = storage[string_data_slot+curr_slot]
                        else:
                            val = w3.eth.get_storage_at(w3.to_checksum_address(contract_addr), string_data_slot+curr_slot)
                            storage[string_data_slot+curr_slot] = val
                            time.sleep(0.25)
...
def extract_array_data(cont_addr, var, all_contracts, contract_abi, all_vars, key_approx_results, tx_arg_details, slots_and_data, all_slots, w3):
    ...
    if levels > 1:
        for i in range(0, levels-1):
            ...
            for q in range(0, len(tmpc)):
                ...
                if slot in storage.keys():
                    arr_length_raw = storage[slot]
                else:
                    arr_length_raw = w3.eth.get_storage_at(
                        w3.to_checksum_address(cont_addr), slot)
                    storage[slot] = arr_length_raw
                    time.sleep(0.25)
                array_length = w3.to_int(arr_length_raw)
    ...
    for key_details in tmpc:
        if key_details[1] in storage.keys():
            g_raw = storage[key_details[1]]
        else:
            g_raw = w3.eth.get_storage_at(
                w3.to_checksum_address(cont_addr), key_details[1])
            storage[key_details[1]] = g_raw
            time.sleep(0.25)
        g = w3.to_int(g_raw)
...
def extract_contract_state(cont_name, source_code, cont_addr, compiler_version, net, transactions = None, internal_transactions = None):
    """
    ...
    if transactions == None:
        all_transactions = get_transactions(cont_addr, all_transactions, TRANSACTION_LINK, BLOCK_SCANNER_API_KEY, w3.eth.block_number)
    else:
        all_transactions = transactions
    
    if internal_transactions == None:
        all_transactions += get_internal_transactions(cont_addr, all_transactions, INTERNAL_TRANSACTION_LINK, BLOCK_SCANNER_API_KEY, w3.eth.block_number)
    else:
